...
namespace MacroRuntime_TEST
{
   ...
   public class IntrinsicProcedure_TEST
   {
      ...
      public void Equals_TEST()
      {
         ...
         var intrinsicProcedure1 = new ExpressionEvaluator(context).Evaluate((ExpressionBase)new MacroParser().Parse("move"));
         var intrinsicProcedure2 = new ExpressionEvaluator(context).Evaluate((ExpressionBase)new MacroParser().Parse("move"));
         Assert.AreEqual(intrinsicProcedure1, intrinsicProcedure2);

         intrinsicProcedure2 = new ExpressionEvaluator(context).Evaluate((ExpressionBase)new MacroParser().Parse("position"));
         Assert.AreNotEqual(intrinsicProcedure1, intrinsicProcedure2);

         intrinsicProcedure2 = new ExpressionEvaluator(new RuntimeContext(IntPtr.Zero)).Evaluate((ExpressionBase)new MacroParser().Parse("pause"));
         Assert.AreNotEqual(intrinsicProcedure1, intrinsicProcedure2);
      }
   }
}
